...
BOARDS_SHIELDS=$(awk '!/^\s*#/ && /- board:/ { board=$3 } !/^\s*#/ && /shield:/ { shield=$2; if (board != "") { printf "%s,%s ", board, shield } } END { print "" }' "$HOST_CONFIG_DIR/build.yaml")
...
then
    ...
    EXTRA_MODULE_DIR="$DOCKER_CONFIG_DIR"
...
compile_board () {
    ...
    then
        ...
        echo
        cat "$LOGFILE"
        echo "$(tput setaf 1)Error: $1_$2 failed$(tput sgr0)"
    fi
}
...
if [[ $MULTITHREAD = "true" ]]; then
    ...
    for pair in $BOARDS_SHIELDS
    do
        board=$(echo $pair | sed 's/,.*//')
        shield=$(echo $pair | sed 's/.*,//')
